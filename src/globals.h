...
// Update.begin() is also deferred to loop(): in ESP8266 Arduino core 3.x it
// erases a flash sector and calls yield() internally, which panics in SYS context.
struct OtaAsyncBuf {
  ...
  volatile bool     beginNeeded   = false;   // true until loop() calls Update.begin()
  ...
  uint32_t          maxSize       = 0;       // passed to Update.begin() by loop()
